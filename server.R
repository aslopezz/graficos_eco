server <- function(input, output, session) {
  ...
  output$resumen_carga <- renderUI({
    ...
    tags$div(
      ...
      sprintf(
        " %d registros · %d columnas · %d especies únicas · abundancia total: %d",
        nrow(df),
        ncol(df),
  ...
  datos_graficos <- eventReactive(input$run_graficos, {
    ...
    if (!is.null(input$filtro_clase) &&
        input$filtro_clase != "Todas")
      df <- df[df$CLASE == input$filtro_clase, ]
    if (!is.null(input$filtro_protocolo) &&
        input$filtro_protocolo != "Todos")
  ...
  output$estacion_ui <- renderUI({
    ...
    selectInput("estacion", "Estación", estaciones)
  })
  
  radar_data <- eventReactive(input$run_radar, {
    datos_aereo() %>%
  ...
  crear_radar <- reactive({
    ...
    direcciones <- c("N", "NO", "O", "SO", "S", "SE", "E", "NE")
    
    radar <- tibble(dir_destino = direcciones) %>%
    ...
    radar_final <- rbind(rep(max_global, 8), rep(0, 8), valores)
  ...
  output$graf_radar <- renderPlot({
    ...
    fmsb::radarchart(
      ...
      cglcol = "grey80",
      axislabcol = "black",
      caxislabels = seq(0, max(radar[1, ]), length.out = 5)
    )
  ...
  datos_espaciales <- eventReactive(input$run_espacial, {
    ...
    validate(
      need(
        input$fecha_ter_campana >= input$fecha_ini_campana,
        "La fecha de término debe ser igual o posterior a la fecha de inicio."
      )
    )
    
    withProgress(message = "Generando capa espacial...", value = 0, {
      incProgress(0.3, detail = "Procesando coordenadas...")
  ...
  output$mapa_espacial <- renderPlot({
    ...
    comuna <- sf::st_transform(comuna, 4326)
    
    tiene_geometria <- !sf::st_is_empty(sf::st_geometry(capa))
    
    puntos <- capa[tiene_geometria, , drop = FALSE]
    
    if (nrow(puntos) > 0) {
      puntos <- sf::st_transform(puntos, 4326)
    }
    ...
    if (nrow(comuna) > 0) {
      ...
      p <- p +
        coord_sf(datum = sf::st_crs(4326), expand = TRUE)
    }
    ...
    p +
      ...
      labs(
        title = paste(input$comuna_mapa, "-", input$region_mapa),
        subtitle = paste("Puntos con coordenadas:", nrow(puntos)),
        x = "Longitud",
        y = "Latitud",
